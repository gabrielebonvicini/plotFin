#' Fama French (Five factors) and CAPM Estimation
#'
#' A function estimating the parameters of the Fama-French five factors model and the coefficients
#' of the CAPM model. The Fama-French five factors are: the market factor, SMB (Small Minus Big),
#' HML (High Minus Low), RMW (Robust Minus Weak) , CMA (Conservative Minus Aggresive)
#'
#' @export
#'
...
ff5.Capm.estimation <- function (ticker, from, to ) {
  ...
  ff_data <- read.csv(ff_data, skip = 3) #SOURCE: https://www.codingfinance.com/post/2018-05-31-download-ff-data-in-r/
  ...
  #Now we shift the X1 column (with the dates) in the left
  rownames(ff_data) <- ff_data[,1]
  ff_data <- data.frame(ff_data)

  #Now we can remove the X1 column
  ff_data <- ff_data[ , -1]
  ...
  #the table
  rownames(regression_merged) <- c("FF Intercept", "Market factor", "SMB", "HML", "RMW",
                                   "CMA",
  ...
  if ( is.character(ticker) == T ) { ######### IF
    regression_merged %>%
      ...
      pack_rows("FF 3 Factors Model", 1, 6 ) %>%
      pack_rows("CAPM Model", 7 , 8) %>%
      print()

    ##And an output of the data in the envinroment
    colnames(y) <- c( "Date", print( paste0(ticker, " returns") ) )
    assign("FF_5_factors_data", ff_data, envir = .GlobalEnv)
